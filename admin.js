  ...
  function renderSitters(sitters) {
    ...
    sittersList.innerHTML = sitters.map(function (s) {
      ...
      return (
        ...
        smileIdSummary(s) +
        '<div class="dash-docs">' + docLink('id_document', s.id, s.has_id_document, 'sitters', 'ID / passport document') + docLink('proof_of_address', s.id, s.has_proof_of_address, 'sitters', 'Proof of address') + docLink('selfie', s.id, s.has_selfie, 'sitters', 'Selfie photo') + docLink('police_clearance', s.id, s.has_police_clearance, 'sitters', 'AFIS check or police clearance certificate') + docLink('child_protection_clearance', s.id, s.has_child_protection_clearance, 'sitters', 'Child Protection Register clearance') + (s.id_type === 'passport' ? docLink('foreign_police_clearance', s.id, s.has_foreign_police_clearance, 'sitters', 'Foreign police clearance') : '') + '</div>' +
